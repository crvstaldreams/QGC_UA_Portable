...
void CompassTelemetryController::_set(int i,double x,double y) {
    if (qFuzzyIsNull(x) && qFuzzyIsNull(y)) return;
...
void CompassTelemetryController::_message(const mavlink_message_t& m) {
    switch(m.msgid) {
    case MAVLINK_MSG_ID_RAW_IMU: { mavlink_raw_imu_t v; mavlink_msg_raw_imu_decode(&m,&v); _set(0,v.xmag,v.ymag); break; }
    case MAVLINK_MSG_ID_SCALED_IMU2: { mavlink_scaled_imu2_t v; mavlink_msg_scaled_imu2_decode(&m,&v); _set(1,v.xmag,v.ymag); break; }
    case MAVLINK_MSG_ID_SCALED_IMU3: { mavlink_scaled_imu3_t v; mavlink_msg_scaled_imu3_decode(&m,&v); _set(2,v.xmag,v.ymag); break; }
    default: break;
    }
}
